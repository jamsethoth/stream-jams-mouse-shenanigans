...
public class CPHInline
{
    public bool Execute()
    {
        string profile = CPH.GetGlobalVar<string>("mouseShenanigans.localControl.doubleRightProfile", true);
        if (string.IsNullOrWhiteSpace(profile))
        {
            profile = "double-right";
        }

        return CallLocalControl("Select Double Right", "POST", "/api/v1/profiles/select", CreateProfileBody(profile));
    }
