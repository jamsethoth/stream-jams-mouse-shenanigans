...
public class CPHInline
{
    public bool Execute()
    {
        string profile = CPH.GetGlobalVar<string>("mouseShenanigans.localControl.horizontalProfile", true);
        if (string.IsNullOrWhiteSpace(profile))
        {
            profile = "horizontal-inversion";
        }

        return CallLocalControl("Select Horizontal Inversion", "POST", "/api/v1/profiles/select", CreateProfileBody(profile));
    }

    private static string CreateProfileBody(string profile)
    {
        return "{\"name\":\"" + EscapeJson(profile) + "\"}";
    }
